...
interface IRadioOptions {
  ...
  questionContent?: any[]; // Add this prop
}
...
const RadioOptions: FC<IRadioOptions> = ({
  ...
  const { control, register, watch, setValue, trigger } = useFormContext();
  ...
  useEffect(() => {
    console.log('qqq Here fields', fields);
  }, [fields]);
  ...
  // Add useEffect to handle radioOptions initialization
  useEffect(() => {
    const currentRadioOptions = watch('radioOptions');
    console.log('qqq Here currentRadioOptions in RadioOptions', currentRadioOptions);

    if (!currentRadioOptions || currentRadioOptions.length === 0 || currentRadioOptions.filter((item: any) => item.text).length === 0) {
      console.log('qqq running in RadioOptions', currentRadioOptions);
      const defaultRadioOptionsData = questionContent || [{ text: '', isChecked: false, remarks: false }];
      setValue('radioOptions', defaultRadioOptionsData);
  ...
  const handleRadioChange = (selectedIndex: string) => {
    fields.forEach((_, index) => {
      ...
      mode === 'answer' && trigger('radioOptions');
    });
  };
  ...
  return (
    <VStack spacing={4} w="100%" justify="flex-start">
          ...
          <FormLabel {...labelStyles} mx={isAnswerMode && '0'} onClick={() => setEditMode(true)}>
            {question && !isAnswerMode && isEditMode && !isDisabledOnEditMode ? (
              ...
              <Box display={'flex'} alignItems={'center'} justifyContent={'space-between'}>
                <Text w="100%" p={isAnswerMode ? '20px' : ''}>{displayIndex ? `${displayIndex}. ` : ''}{title}{isRequired ? <span style={{ color: 'red' }}> *</span> : null}</Text>
                {files?.url ?
        ...
        <RadioGroup onChange={handleRadioChange}>
          {fields.map((item, index) => (
            <VStack key={item.id} alignItems="center" mb="2" gap="15px" w="100%">
              <HStack mr="auto">
                <Radio
                  isDisabled={mode !== 'answer' || ((location.pathname.includes('survey-list') && location.pathname.includes('admin')) || (location.pathname.includes('survey-list') && location.pathname.includes('manager')))}
                  value={mode !== 'view' ? index.toString() : undefined}
                  isChecked={watch(`radioOptions[${index}].isChecked`)}
                />
                <FormControl variant="floating">
                  <Input
                    ...
                    placeholder=""
                    isDisabled={isAnswerMode || isDisabledOnEditMode}
                  />
      ...
      {isAnswerMode && hasRemarks && <Box w="100%" display='flex' px={isAnswerMode ? '20px' : ''}>
        <FormControl variant='floating'>
          <Input
