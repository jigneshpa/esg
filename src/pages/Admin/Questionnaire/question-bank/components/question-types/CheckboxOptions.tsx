...
const CheckboxOptions: FC<ICheckboxOptions> = ({
  ...
  const { control, register, watch, trigger, setValue } = useFormContext();
  ...
  useEffect(() => {
    const currentCheckboxOptions = watch('checkboxOptions');
    if (!currentCheckboxOptions || currentCheckboxOptions.length === 0 || currentCheckboxOptions.filter((item: any) => item.text).length === 0) {
      const defaultCheckboxOptionsData = questionContent || [{ text: '', isChecked: false, remarks: false }];
      setValue('checkboxOptions', defaultCheckboxOptionsData);
      trigger('checkboxOptions');
    }
            ...
            <HStack mr="auto">
              <Checkbox
                ...
                {...register(`checkboxOptions[${index}].isChecked`, {
                  onChange: () => setRegistrationComplete(true)
                })}
              />
